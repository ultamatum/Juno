...
namespace Juno {
	...
	std::string FileDialogs::OpenFile(const char* filter)
	{
		...
		if (fp == NULL)
			JUNO_CORE_ERROR("Null file pointer in FileDialogs::OpenFile().\n");
	...
	bool FileDialogsUtilityCheck()
	{
		...
		if (fgets(path, LINUX_PATH_MAX, fp) == NULL)
		{
			JUNO_CORE_WARN("Zenity not found in path. See Hazel pre-requisites to install.");
			pclose(fp);
			return false;
		}
		pclose(fp);
