		...
		public:
			...
			void BeginSession(const std::string& name, const std::string& filepath = "results.json")
			{
				...
				if (m_CurrentSession)
				{
					...
					{
						JUNO_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
					}
					InternalEndSession();
				}
...
#define JUNO_PROFILE 1
	...
	// could mark the wrong one in your editor!
		...
		#define JUNO_FUNC_SIG __FUNC__
	#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
		#define JUNO_FUNC_SIG __func__
	#elif defined(__cplusplus) && (__cplusplus >= 201103)
		#define JUNO_FUNC_SIG __func__
	#else
		#define JUNO_FUNC_SIG "JUNO_FUNC_SIG unknown!"
	#endif
	...
	#define JUNO_PROFILE_END_SESSION()
	#define JUNO_PROFILE_SCOPE(name)
