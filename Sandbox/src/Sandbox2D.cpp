...
void Sandbox2D::OnAttach()
{
	...
	m_SpriteSheet = Juno::Texture2D::Create("assets/game/textures/RPGpack_sheet_2X.png");

	m_TextureStairs = Juno::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 7, 7 }, { 128, 128 });
	m_TextureBarrel = Juno::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 8, 2 }, { 128, 128 });
	m_TextureTree = Juno::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 2, 1 }, { 128, 128 }, { 1, 2 });

	m_MapWidth = s_MapWidth;
	m_MapHeight = strlen(s_MapTiles) / m_MapWidth;

	s_TextureMap['D'] = Juno::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 6, 11 }, { 128, 128 });
	s_TextureMap['W'] = Juno::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 11, 11 }, { 128, 128 });
...
void Sandbox2D::OnUpdate(Juno::Timestep ts)
{
	...
	for (uint32_t y = 0; y < m_MapHeight; y++)
	{
		for (uint32_t x = 0; x < m_MapWidth; x++)
		{
			...
			Juno::Ref<Juno::SubTexture2D> texture;
